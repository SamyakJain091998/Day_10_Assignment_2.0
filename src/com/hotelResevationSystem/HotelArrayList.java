...
public class HotelArrayList {
	...
	public void printHotelMap() {
		...
		for (Map.Entry<String,Integer> entry : hotelMap.entrySet())  
			if(entry.getValue() < minValue) minValue = entry.getValue();
		for (Map.Entry<String,Integer> entry : hotelMap.entrySet()) {
		...
		if(count == 1) {
			...
			System.out.println("Hotels are : " + storeHotelName + " and total rate = " + minValue);
		}
	}
		...
		if(typeOfCustomer == 1) {
			...
			while(itr.hasNext()) {
				...
				if(diff == days) {
					...
					localSum = localSum + noOfWeeks*(5*(h.getHotelRateRegularWeekDay()) + 2*(h.getHotelRateRegularWeekEnd()));
				}
...
//		System.out.println(Arrays.asList(hotelMap));
//		System.out.println();
